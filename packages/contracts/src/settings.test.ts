...
describe("ClientSettings sent message scrolling", () => {
  it("defaults sent-message top scrolling off", () => {
    expect(decodeClientSettings({}).autoScrollSentMessageToTop).toBe(false);
  });

  it("accepts sent-message top scrolling updates", () => {
    expect(
      decodeClientSettingsPatch({ autoScrollSentMessageToTop: true }).autoScrollSentMessageToTop,
    ).toBe(true);
  });
});
